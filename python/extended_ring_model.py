...
This module contains the full specification for the ring model from
the paper. The model is based on data from Pisokas et al. (2020), Goulard
et al. (2021), Fisher et al. (2019), Kim et al. (2019), Kim et al. (2017),
and Seelig and Jayaraman (2015).

[Biology]
Pisokas et al. (2020) provide a comparative study of the locust and fruit-fly
head-direction circuits. They find that the two are functionally equivalent
with some differences in dynamics. Our model represents a simplified hybrid
of the locust and fruit-fly circuits (see the paper for more details and
justification).

[Usage]
The model is fairly simple to use. Instantiate the class then
call the initialise() method to generate an stable initial state in
the network. Once you have done this, you can interact with the model
using the update_state() method. This is designed such that you can
wrap the model in a simulation routine and update it on each timestep.

[Plasticity]
Once the model is initialised, plasticity can be set using the
plasticity keyword in the update_state() method
(i.e. plasticity=True). This will enable weight changes using
update_weights() at the prescribed learning rate, but will also shift
the balance of input to the E-PGs (decreasing relative R input). See
the paper or get in touch for more details.

For further information, please see the comments on each method
or get in touch if you have any questions.

References:
Pisokas et al. (2020) - The head direction circuit of two insect species
Goulard et al. (2021) - A unified mechanism to support innate and learned use of visual landmark guidance in insects
...
Kim et al. (2017)  - Ring attractor dynamics in the Drosophila central brain
Seelig and Jayaraman (2015) - Neural dynamics for landmark orientation and angular path integration

"""

from dict_key_definitions import rmkeys, decodekeys, inputkeys
import numpy as np
...
class RingModel():
    ...
    def pen_output(self, self_motion=0):
        ...
        self.input_dict[inputkeys.pen] = pen_input
        if self.show_inputs:
    ...
    def peg_output(self):
        ...
        self.input_dict[inputkeys.peg] = peg_input
    ...
    def epg_output(self,
        ...
        self.input_dict[inputkeys.epg] = total_input
    ...
    def d7_output(self):
        ...
        self.input_dict[inputkeys.d7] = total_input
    ...
    def autotune(self, iterations=10, duration=100, window=0):
        """
        [Legacy] - Tuning the network parameters by hand was cumbersome
        and this was one of two attempts to semi-automate the
        process. This routine attempted to find rate parameters which
        gave a sensible range of neural responses within each neuron
        type.

        Autotune rate parameters for given weight relationship.

        :params iterations: Specify how many autotuning iterations we should run
        :params duration: The duration of each autotuning iteration
        :params window: The duration of a simulation window (apparently unused)
        ...
        for i in range(iterations):
            ...
            for t in range(duration):
                w1=0.5
                w2=0.5
                change = np.random.normal(loc=0, scale=20)
                c1 += change
                c2 += change

                self.update_state(c1, c2, change, w1=w1, w2=w2)

                # Track max and min for each neuron type over entire scenario
                for key in self.input_dict.keys():
                    max_test = max(self.input_dict[key])
                    min_test = min(self.input_dict[key])
                    if max_test > maxs[key]:
                        maxs[key] = max_test

                    if mins[key] == None:
                        mins[key] = min_test
                    elif min_test < mins[key]:
                        mins[key] = min_test

            # Given max and min over entire sequence, work out a tuning function
            # which covers this region.
            self.epg_slope, self.epg_bias = act_parameters(max=maxs[inputkeys.epg],
                                                           min=mins[inputkeys.epg])
            self.peg_slope, self.peg_bias = act_parameters(max=maxs[inputkeys.peg],
                                                           min=mins[inputkeys.peg])
            self.pen_slope, self.pen_bias = act_parameters(max=maxs[inputkeys.pen],
                                                           min=mins[inputkeys.pen])
            self.d7_slope, self.d7_bias = act_parameters(max=maxs[inputkeys.d7],
                                                         min=mins[inputkeys.d7])
...
def act_parameters(a=None, max=None, min=None):
    """
    [Legacy] - Determine activation parameters based on minimum and
    maximum inputs. This function is badly written but a used in the
    autotuning method above, it should work as intended. Passing an
    array will fail or produce undefined behaviour depending on when
    and where this method is called.

    :param a: Array-like, neural population inputs (bug, non functional)
    :param max: The maximum expected input to the neuron
    :param min: The minimum expected input to the neuron
    :return: The slope and bias of tuning function
    """

    # Use defined max and min
    max_x = max
    min_x = min

    if a != None: # If array passed, work from there
        max_x = max(x)
        min_x = min(x)

    if (max_x == None or min_x == None) and a == None:
        return 0,0 # Fail quitely

    # Desired centrepoint of the sigmoid
    midpoint = 0.5*(max_x - min_x) + min_x
    width = max_x - min_x

    # Slope which covers the desired range
    a = 10/width
    # Bias which centres the sigmoid on the midpoint of activation
    b = a * midpoint
